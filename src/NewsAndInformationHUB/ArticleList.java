...
public class ArticleList extends javax.swing.JFrame {
     ...
     */
     public ArticleList() {
        ...
        setSize(840, 540); // Set the window size
        setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);  // Exit the application on close

        // Fetch the latest videos
        NewsFeed newsFeed = new NewsFeed();
        List <Article> article = newsFeed.getLatestArticles();

        // Create a JList to display videos
        JList <Article> list = new JList<>(article.toArray(new Article[0]));
        list.setCellRenderer(new ShortArticleRenderer());                         // Custom rendering for clickable links
        ...
        list.addMouseListener(new MouseAdapter() {
            ...
            public void mouseClicked(MouseEvent e) {
                if (e.getClickCount() == 1) {  // Single click event
                    ...
                    Article selectedArticle = list.getModel().getElementAt(index);    // Get the selected video
                    openWebpage(selectedArticle.getUrl());
                }
            }
        });
    ...
    private class ShortArticleRenderer extends DefaultListCellRenderer {
        @Override
    ...
    public static void main(String args[]) {
        ...
        /* Create and display the form */
            SwingUtilities.invokeLater(() -> new ArticleList());
